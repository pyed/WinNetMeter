...
- [x] G1: Native x64 Release build succeeds with static CRT (/MT) producing NetworkMonitorLite.exe
...
- [x] G2: Compiler warnings are zero under strict flags (/W4 /WX /permissive- /std:c++20)
...
- [x] G3: Network interface enumeration correctly queries and filters physical interfaces (Ethernet, 802.11, Gigabit)
...
- [x] G4: Adapter selection uses stable interface identity (InterfaceIndex / LUID) rather than array indices
...
- [x] G5: 64-bit RX and TX counters correctly accumulate and handle full 64-bit octet values
...
- [x] G6: Monotonic clock (QPC) measures actual elapsed time for throughput calculation without assuming fixed 1000ms ticks
...
- [x] G7: Counter reset, adapter down, and adapter reconnect events are handled gracefully without invalid negative spikes or crashes
...
- [x] G8: Main window layout, controls, dark theme, speed formatters, and author link match C# behavioral specifications
...
- [x] G9: Tray icon renders dynamic speed text and context menu provides Show Window, Settings, Show Taskbar Widget, and Exit
...
- [x] G10: Taskbar overlay widget supports drag, topmost positioning, opacity, border, and dynamic color/font settings
...
- [x] G11: Settings persistence accurately loads and saves INI configuration under %APPDATA%\NetworkMonitorLite\settings.ini
...
- [x] G12: Native Settings UI allows choosing colors, fonts, and live preview matching the original settings form
...
- [x] G13: Tray Exit executes clean shutdown terminating timers, windows, tray icon, and exiting with zero leftover background processes
...
- [x] G14: Explorer restart is handled via TaskbarCreated window message to restore the tray icon
...
- [x] G15: Per-Monitor DPI Awareness V2 is initialized and handles WM_DPICHANGED rescaling
...
- [x] G16: Native GDI and Win32 handles (HICON, HFONT, HBRUSH, HDC, HMENU) have explicit lifetime management with zero resource leaks
...
- [x] G17: PE imports inspection confirms zero VC runtime DLLs, zero .NET runtime, and only standard Windows system DLLs
...
- [x] G18: Security and privacy audit confirms zero telemetry, zero updaters, zero shell execution, and zero elevation requirements
...
- [x] G19: Outbound network audit confirms zero unsolicited socket/HTTP connections
...
- [x] G20: Repository cleanliness confirms obsolete Qwen test/debug artifacts are removed and build products are cleanly located in out/
